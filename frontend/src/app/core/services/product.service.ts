...
const MOCK_PRODUCTS: Product[] = [
  {
    ...
    unitPrice: 149.99,
    stock: 30,
    description: 'Beauty & Grooming ürünü',
  ...
  },
  {
    ...
    unitPrice: 299.0,
    stock: 15,
    description: 'Soghaat koleksiyonu',
    category: MOCK_CATEGORIES[6],
    categoryName: 'Soghaat',
    stockQty: 15,
  },
  {
    id: 6,
    sku: 'UK_Namkino_200G',
    name: 'UK_Namkino All In One 200 Gms',
    unitPrice: 89.5,
    stock: 50,
    description: 'Doğal ürün',
    category: MOCK_CATEGORIES[6],
    categoryName: 'Soghaat',
    stockQty: 50,
  },
  {
    id: 7,
    sku: 'UK_Namkino_400G',
    name: 'UK_Namkino Mix Nimco 400 Gms',
    unitPrice: 120.0,
    stock: 40,
  ...
  },
  {
    id: 9,
    sku: 'GFC_Myga_Cross',
    name: 'GFC Pedestal Myga Cross Base',
    unitPrice: 450.0,
    stock: 12,
  ...
  {
    id: 13,
    sku: 'kcc_glamour_deal',
    name: 'kcc_glamour deal',
    unitPrice: 159.0,
    stock: 35,
    description: 'Güzellik ürünü',
    category: MOCK_CATEGORIES[0],
    categoryName: 'Beauty & Grooming',
    stockQty: 35,
  },
  {
    id: 14,
    sku: 'Assetmen_MD346M',
    name: 'Assetmen MD-346-M',
    unitPrice: 340.0,
    stock: 8,
    description: 'Erkek modası',
  ...
  },
  {
    id: 15,
    sku: 'cr_DATES_CASHEW',
    name: 'cr-DATES WITH CASHEW-400 GM',
    unitPrice: 95.0,
    stock: 60,
    description: 'Özel tarihi ürün',
    category: MOCK_CATEGORIES[6],
    categoryName: 'Soghaat',
    stockQty: 60,
  },
  {
    id: 16,
    sku: 'UK_Gift_Box_Dry',
    name: 'UK_Gift Box Mix Dry Fruit Sweets 500 Gms',
    unitPrice: 210.0,
    stock: 22,
    description: 'Hediye kutusu',
    category: MOCK_CATEGORIES[6],
    categoryName: 'Soghaat',
    stockQty: 22,
  },
  ...
  {
    id: 23,
    sku: 'D_Lend_Helping_Hand',
    name: 'D Lend a Helping Hand',
    unitPrice: 115.0,
    stock: 3,
  ...
  {
    id: 24,
    sku: 'Mochika_M0001112_12',
    name: 'Mochika_M0001112-12',
    unitPrice: 260.0,
    stock: 10,
    description: 'Erkek modası',
    category: MOCK_CATEGORIES[4],
...
export class ProductService {
  ...
  getProducts(): Observable<Product[]> {
    ...
    return this.http.get<any>(`${this.base}/products`).pipe(
      map((res) => {
        ...
        if (res?._embedded?.products)
          return this.mapProducts(res._embedded.products);
  ...
  getCategories(): Observable<Category[]> {
    ...
    return this.http.get<any>(`${this.base}/categories`).pipe(
      map((res) => {
        let rawCats: any[] = [];
        if (Array.isArray(res)) rawCats = res;
        else if (res?._embedded?.categories) rawCats = res._embedded.categories;
  ...
  searchProducts(query: string): Observable<Product[]> {
    ...
    return this.http
      .get<any>(
        `${this.base}/products/search?name=${encodeURIComponent(query)}`,
      )
      .pipe(
  ...
  getProductsByCategory(categoryId: number): Observable<Product[]> {
    if (USE_MOCK) {
      return of(MOCK_PRODUCTS.filter((p) => p.category?.id === categoryId));
    }

    return this.http
      .get<any>(`${this.base}/products/category/${categoryId}`)
      .pipe(
        map((res) => (Array.isArray(res) ? this.mapProducts(res) : [])),
        catchError((err) => {
          console.error(
