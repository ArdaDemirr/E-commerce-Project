...
public class UserService {
    ...
    public void deleteUser(Long id) {
        ...
        if ("CORPORATE".equalsIgnoreCase(user.getRole())) {
            ...
            jdbcTemplate.update("DELETE FROM reviews WHERE product_id IN (SELECT id FROM products WHERE store_id IN ("
                    + storeIdsQuery + "))");

            // Delete order_items linked to products belonging to the store
            jdbcTemplate
                    .update("DELETE FROM order_items WHERE product_id IN (SELECT id FROM products WHERE store_id IN ("
                            + storeIdsQuery + "))");

            // Delete order_items of orders placed TO the store
            jdbcTemplate.update("DELETE FROM order_items WHERE order_id IN (SELECT id FROM orders WHERE store_id IN ("
                    + storeIdsQuery + "))");
